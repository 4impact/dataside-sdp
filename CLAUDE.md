...
- `PermanentSdpException` → skips retry, goes straight to DLQ (4xx, deserialization failures, missing required fields).
...
**SDP API wire format** — All calls to SDP use `Content-Type: application/x-www-form-urlencoded` with a single field `input_data=<URL-encoded JSON>`. The JSON is wrapped in `{"request": <payload>}`. This is a ManageEngine SDP v3 API requirement. The `buildInputData()` method in `ServiceDeskPlusClientImpl` handles this wrapping and encoding.
...
- `ServiceDeskPlusClientImplTest` — starts a `WireMockServer` on a dynamic port and constructs `ServiceDeskPlusClientImpl` directly with the WireMock base URL
